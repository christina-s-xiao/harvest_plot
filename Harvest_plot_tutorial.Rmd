...
In systematic reviews where meta-analysis is not possible, visualisations are important to show patterns in the data more efficiently than using text [(Cochrane Handbook).](https://training.cochrane.org/handbook/current/chapter-12#section-12-3) Harvest plots are one way to do this, they have been defined by [Ogilvie et al., 2009](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/1471-2288-8-8) as:
<br>
...
We aim to demonstrate how to create a simple harvest plot and how this can be extended where there are multiple outcomes and exposures of interest. Our examples come from "Shifting towards healthier transport: carrots or sticks? Systematic review and meta-analysis of population-level interventions" [Xiao et al., 2022](https://www.thelancet.com/journals/lanplh/article/PIIS2542-5196(22)00220-0/fulltext). This study aimed to compare the effectiveness of carrot, stick, and combined carrot-and-stick interventions in changing different transport behaviors. 

**maybe a sentence to say: "we do not discuss the choice of methods but only discuss the merits of Harvest plots where it's pertinant to creating the plot" no don't include** 

**This helps to automate the plotting while allowing the user to change which dimensions are represented by different study characteristics.**
<br><br>
...
**RP comment: i wonder whether to just do this and save the data as a .Rdata file. We can incorporate variable type (factor etc.) in the list above. It would just allow us to focus on the important buts (i'm not 100% about this because this is clearly an immportant part of getting it to look right)**
...
```{r}
dc <- data[data$outcome_mode == 'Driving' & data$cs_category == 'Carrot' , ]

ggplot(dc, aes(y=quality_value, x=position)) + geom_bar(stat="identity") +
...
#ggplot(drive, aes(fill=stat_sig, y=quality_value, x=position, label = study_id)) + 
