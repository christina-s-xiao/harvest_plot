...
df = df %>%
  ...
  arrange(desc(quality), stat_sig, desc(n_interventions), .by_group = TRUE) # Remember to change the variables quality, stat_sig, and n_interventions if you are categorising interventions differently.
...
drive_a<- ggplot(drive, aes(fill=stat_sig, y=quality, x=position, label = study_id)) + 
  ...
  scale_fill_manual(values=cbPalette, na.translate=FALSE) + # Adding colours to the bars
